        ...
        <header className="relative pt-32 pb-16 md:pt-40 md:pb-24 overflow-hidden">
          ...
          {post.image && (
            <div className="absolute inset-0">
              <Image
                src={post.image}
                alt={`${post.title} - Artículo de salud Clínica Hispana Cruz 2 Houston TX`}
                fill
                      ...
                      {relatedPost.image && (
                        <div className="relative h-40 overflow-hidden">
                          <Image
                            ...
                            alt={`${relatedPost.title} - Blog Clínica Hispana Cruz 2 Houston`}
                            fill
...
function parseMarkdown(markdown: string, locale: string = "es"): string {
  ...
  let html = withTables
    ...
    // Italic
    .replace(/\*(.*?)\*/gim, '<em>$1</em>')
    // Links: los internos ("/...") se prefijan con el locale activo para no
    // mandar al lector de /en a la versión en español
    .replace(/\[(.*?)\]\((.*?)\)/gim, (_match, text: string, href: string) => {
      const localizedHref =
        locale !== "es" && href.startsWith("/") && !href.startsWith(`/${locale}/`)
          ? `/${locale}${href}`
          : href;
      return `<a href="${localizedHref}">${text}</a>`;
    })
    // Unordered lists
    .replace(/^- (.*$)/gim, '<li>$1</li>')
    // Ordered lists
